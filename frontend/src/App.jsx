...
function App() {
  ...
  const finalClaim = String(customClaim || claim || "");
  ...
  try {
    ...
    if (data.success) {
      setPhase("Refining with sources...");
      setMessage("Quick AI response ⚡");
      fetchNews();
    } else {
  ...
  try {
    ...
    if (data.success) {
      ...
      setMessage("Verifying extracted text...");

      await handleSubmit(data.extractedText);
    } else {
  ...
  try {
    ...
    if (data.success) {
      setFileText(data.extractedText);
  ...
  return (
    <div className="app">
      ...
      <h1>TruthLens-AI</h1>
      <p>Fake News & Misinformation Detector</p>
          ...
          analysisArray.map((claimItem, i) => (
            <div key={i} className="claim-card">
              ...
              <p>
                ...
                <span className={`badge ${claimItem.result?.toLowerCase()}`}>
                  {claimItem.result}
                </span>
              </p>

              <p>
                <strong>Explanation:</strong> {claimItem.explanation}
              </p>
